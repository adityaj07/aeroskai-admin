...
export const CompanySubscriptionHeaderBanner = ({ details }) => {
  return (
    <div className="rounded-2xl bg-[#1565C00D] p-4 dark:bg-[#1565C01A]">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-[20px] font-semibold text-[#1F1E1F] dark:text-white">{details.username}</p>
            <p className="text-[13px] text-[#6F7680] dark:text-[#A9B0BA]">{details.companyName}</p>
          </div>
        </div>

        <div className="flex flex-wrap items-center gap-2">
          <SubscriptionStatusBadge status={details.status} />
          {details.foundingCompany && (
            <div className="flex items-center gap-1 rounded-lg bg-[#1565C01A] px-2 py-1 text-[12px] text-[#1565C0]">
