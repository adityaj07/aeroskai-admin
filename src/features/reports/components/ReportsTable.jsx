...
const getColumns = (onOpenDetails) => [
  {
    ...
    cell: ({ row }) => (
      <span className="font-medium text-[#0C1014] dark:text-white">{row.original.id}</span>
    ),
  },

  {
    accessorKey: 'reportedType',
    header: 'Reported Type',
  },

  {
    accessorKey: 'reportsCount',
    header: 'Number of Users Reported',
  },
...

  {
    ...
    cell: ({ row }) => {
      ...
      const label =
        status === 'Pending' ? 'Review' : status === 'Under Review' ? 'Take Action' : 'View'
